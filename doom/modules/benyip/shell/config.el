 ;;; benyip/shell/config.el -*- lexical-binding: t; -*-
...
(use-package! term
  ...
  (defun term-mode-prompt-regexp-setup ()
    ...
    (setq-local term-prompt-regexp "^[λX] *"))
...
(use-package! vterm
  :hook ((vterm-mode . vterm-mode-prompt-regexp-setup))
  :config
  (defun vterm-mode-prompt-regexp-setup ()
    "Setup `term-prompt-regexp' for term-mode."
    (setq-local term-prompt-regexp "^[λX] *")))
