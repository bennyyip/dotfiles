...
local function initializeIfNeeded()
    ...
    mp.add_forced_key_binding("shift+f8", "seek-to-start-position", function()
        mp.commandv("seek", startPosition, "absolute")
        mp.command("show-progress")
        updateTrimmingPositionsOSDASS()
    end)
    mp.add_forced_key_binding("shift+f9", "seek-to-end-position", function()
...
-- Static Key Bindings
--
mp.add_key_binding("f8", "trim-set-start-position", setStartPosition)
mp.add_key_binding("f9", "trim-set-end-position", setEndPosition)
