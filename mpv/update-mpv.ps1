# Modified from https://github.com/shinchiro/mpv-packaging/blob/master/mpv-root/installer/updater.ps1
...
function Check-7z {
    if (-not (Get-7z))
    {
        ...
        Invoke-WebRequest -Proxy $proxy -Uri "https://www.7-zip.org/a/7zr.exe" -UserAgent $useragent -OutFile $download_file
    }
    else
    {
...
function Download-Archive ($filename, $link) {
    ...
    Invoke-WebRequest -Proxy $proxy -Uri $link -UserAgent $useragent -OutFile $filename
}
...
function Download-Ytplugin ($plugin, $version) {
    ...
    Invoke-WebRequest -Proxy $proxy -Uri $link -UserAgent $useragent -OutFile $plugin_exe
}
...
function Get-Latest-Ytplugin ($plugin) {
    switch -wildcard ($plugin) {
        "yt-dlp*" {
            ...
            $resp = [xml](Invoke-WebRequest -Proxy $proxy $link -MaximumRedirection 0 -ErrorAction Ignore -UseBasicParsing).Content
        ...
        "youtube-dl" {
            ...
            $resp = Invoke-WebRequest -Proxy $proxy $link -MaximumRedirection 0 -ErrorAction Ignore -UseBasicParsing
